...
interface ClearSubAccountsResult {
    success: boolean;
    message: string;
    subAccountsCleared?: number;
    mainAccountsPreserved?: number;
    error?: string;
}

export const clearSubAccounts = async (
    adminEmail = 'admin@accounting-app.com',
    adminPassword = 'Admin123!'
): Promise<ClearSubAccountsResult> => {
    ...
    try {
        ...
        const mainAccounts = allAccounts.filter(account =>
            mainAccountCodes.includes(account.accountCode)
        );
        ...
        const remainingMainAccounts = remainingAccounts.filter(account =>
            mainAccountCodes.includes(account.accountCode)
        );

        const remainingSubAccounts = remainingAccounts.filter(account =>
            !mainAccountCodes.includes(account.accountCode)
        );
